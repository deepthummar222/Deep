...
const UploadImage = () => {
  ...
  const handleFileChange = async (event) => {
    ...
    .on('httpUploadProgress', (evt) => {
      console.log(evt);
      const imagedata = `https://dating-app-admin-panel.s3.amazonaws.com/${selectedFile.name}`;
      localStorage.setItem('imagedataArray', JSON.stringify(imagedata));
    })
    .send((err) => {
        ...
        <form className="form-container" encType="multipart/form-data">
          <div className="upload-files-container">
            <div className="drag-file-area">
              ...
              <h3 className="dynamic-message"> select fastival poster </h3>
              <label className="label"> or <span className="browse-files"> <input type="file"  accept="image/*" className="default-file-input" onChange={handleFileChange} /> <span className="browse-files-text">browse file</span> <span>from device</span> </span> </label>
            </div>
          </div>
        </form>
      </div>
